...
This is copied almost verbatim from the paint-by-language-model repo,
with a few adjustments for this repository. 

https://github.com/liamlaverty/paint-by-language-model/blob/3da492ea366b4626b9ba171995aa8b0281e0f222/src/paint_by_language_model/vlm_client.py
"""
...
class VLMClient:
    ...
    def __init__(
        ...
        self.load_unload_url = self.base_url.replace("/v1", "")
    ...
    def _build_multimodal_payload(
        ...
        if self.provider == "anthropic":
            ...
            image_data_url = self._encode_image_to_base64(image_bytes, "jpeg")
    ...
    def load_model(self, model_name: str) -> dict:
        """
        Load a model into the VLM server.
        ...
        """
        if self.list_loaded_models() and model_name in self.list_loaded_models():
            logger.info(f"Model {model_name} is already loaded in the VLM server.")
            return {"status": "already_loaded", "instance_id": model_name}
        else: 
            print(f"Model {model_name} is not currently loaded. Attempting to load it now")
        try:
            response = requests.post(
                f"{self.load_unload_url}/api/v1/models/load",
            ...
            raise RuntimeError(f"Failed to load model {model_name} (the model may already be loaded)") from e


    def list_loaded_models(self):
        """
        List models currently loaded in the VLM server.
        ...
        try:
            model_list = []
            response = requests.get(
                f"{self.load_unload_url}/api/v1/models",
                headers=self._build_headers(),
                timeout=self.timeout,
            )
            response.raise_for_status()
            ...
            for model in result.get("models", []):
                # check if a model's loaded_instances is not empty
            ...
            raise RuntimeError("Failed to list models") from e



    def unload_model(self, model_name: str):
        """
        Unload a model from the VLM server.
        ...
        try:
            response = requests.post(
                f"{self.load_unload_url}/api/v1/models/unload",
                json={"instance_id": model_name}
            )
            response.raise_for_status()
        except requests.RequestException as e:
            print(f"Failed to unload model {model_name}: {e}")
            raise RuntimeError(f"Failed to unload model {model_name} (the model may not be loaded)") from e
    ...
    def _encode_image_to_base64(self, image_bytes: bytes, type: str) -> str:
        """
        Encode image bytes to base64 data URL format.

        Args:
            image_bytes (bytes): Raw image bytes
            type (str): Image MIME type (e.g., "png", "jpeg")
